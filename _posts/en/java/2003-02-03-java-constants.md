...
As is similarly the case in all programming languages, keywords in Java cannot be used as variable or constant names, so words like `final`, `int` and we will see `while`, `do` and others cannot be used as variable or constant names. 
