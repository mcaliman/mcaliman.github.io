...
A classical example of recursion is the factorial function: given a number `n`, we denote by `n!` the result of the product of all positive integers from `1` to `n`. By convention, the factorial of `0` (i.e., `0!`) is equal to `1`.
...
```
The `reverse` method makes use of linear recursion and tail recursion—i.e., the recursive call is the last operation performed by the method.
