...
Returning to the concept of the string, there is a special one whose meaning will not appear immediately clear or of any use, but which will play a crucial role later on. Its name is the **empty string**, and commonly in texts it is represented by the lowercase Greek letter epsilon ($\epsilon$) and sometimes by the uppercase Greek letter lambda ($\Lambda$). 
...
In addition to the operations that we can do on strings, there are similar operations on languages. Of particular interest are the closure operations or closures: the positive closure represented by a $+$ sign at the exponent $L^+$, and the Kleene closure represented by the asterisk symbol $L^\*$. Informally, the former is the union of all languages of length 1 to $n$, while the latter includes all languages of length 0 to $n$, so it also includes the string of length 0.
