...
layout: post
title: "Java Array"
excerpt: "Arrays are available in most programming languages because they realise the simplest organisation of complex data structures"
category: Java
date: 2003-03-01
comments: true
share: false
...
Hence, the values of the individual elements are equal to:
...
I can modify the second element of the array, i.e., `var[1]`, with the value `new_value_1` using the instruction:
...
between 0 and `var.length - 1` (where `length` represents the length of the array `var`).
...
element is identified not by a single index, but by a larger number of indices.
...
To initialise the value at position or coordinate `i,j`, I can write:
