...
* from each `v` node it is possible to trace back in time `O(1)` either to its parent (which has index $\lfloor v/d \rfloor$ if $v$ is other than the root) or to any of its children.
